...
public class MainRun extends Application {
	...
	private final double evalue = Double.parseDouble("1e10");
	...
	private final ArrayList<Process> mainProcesses = new ArrayList<Process>();
	...
	public void start(Stage primaryStage) {
		...
		// Input file / folder
		Text inputPrompt = new Text("Input fasta/fastq file or directory containing fasta/fastq files");
...
//		inputField.setText("/home/marco/analyses/insilicoPCR_brucella/fasta");  // Faster for debugging
...
//		inputField.setEditable(false);
		...
		Button inputBrowse = new Button("Select");
		inputBrowse.setPrefSize(Double.MAX_VALUE, Double.MAX_VALUE);
		pane.add(inputBrowse, 31, 4, 4, 2);
		...
		pane.add(refPrompt, 2, 10, 20, 2);


		TextField refField = new TextField();
//		refField.setText("/home/marco/analyses/insilicoPCR_brucella/primers_brucella_spp.fasta");
...
//		refField.setEditable(false);
		...
		Button refBrowse = new Button("Select");
		refBrowse.setPrefSize(Double.MAX_VALUE, Double.MAX_VALUE);
		pane.add(refBrowse, 31, 12, 4, 2);
		...
		Text outPrompt = new Text("Output directory. Path MUST NOT contain spaces");
		...
		pane.add(outputBox, 2, 15, 20, 2);
		
		TextField outField = new TextField();
//		outField.setText("/home/marco/analyses/insilicoPCR_brucella/out");
		outField.setPrefSize(Double.MAX_VALUE, Double.MAX_VALUE);
		outField.setEditable(false);
		pane.add(outField, 2, 17, 28, 2);
		...
		Button outBrowse = new Button("Select");
		outBrowse.setPrefSize(Double.MAX_VALUE, Double.MAX_VALUE);
		pane.add(outBrowse, 31, 17, 4, 2);
		...
		// Threads
		Text threadPrompt = new Text("Threads");
		threadPrompt.getStyleClass().add("prompt");
		threadPrompt.setTextAlignment(TextAlignment.LEFT);
		HBox threadBox = new HBox(10);
		threadBox.setAlignment(Pos.TOP_LEFT);
		threadBox.getChildren().add(threadPrompt);
		pane.add(threadBox, 38, 5, 5, 1);
		...
		pane.add(threadField, 44, 4, 4, 3);
		
		// Mismatches
		Text mismatchPrompt = new Text("Mismatches");
		mismatchPrompt.getStyleClass().add("prompt");
		mismatchPrompt.setTextAlignment(TextAlignment.LEFT);
		HBox mismatchBox = new HBox(10);
		mismatchBox.setAlignment(Pos.TOP_LEFT);
		mismatchBox.getChildren().add(mismatchPrompt);
		pane.add(mismatchBox, 38, 10, 5, 1);
		...
		pane.add(mismatchField, 44, 9, 4, 3);
		
		//Evalue
		Text evaluePrompt = new Text("Evalue");
		evaluePrompt.getStyleClass().add("prompt");
		evaluePrompt.setTextAlignment(TextAlignment.LEFT);
		HBox evalueBox = new HBox(10);
		evalueBox.setAlignment(Pos.TOP_LEFT);
		evalueBox.getChildren().add(evaluePrompt);
		pane.add(evalueBox, 38, 15, 5, 1);
		
		TextField evalueField = new TextField("1e10");
		evalueField.setPrefSize(Double.MAX_VALUE, Double.MAX_VALUE);
		evalueField.setEditable(true);
		pane.add(evalueField, 44, 14, 4, 3);
		
		// Big dialog box to print messages
		outputField = new TextArea();
		outputField.setPrefSize(Double.MAX_VALUE,Double.MAX_VALUE);
		outputField.setEditable(false);
		pane.add(outputField, 2, 21, 46, 22);
		
		// Space under the big dialog box to display alerts
			...
			public void handle(ActionEvent e) {
				...
				if(inputFile == null || outDir == null || primerFile == null) {
					...
					if(inputFile.isDirectory()) {
						...
						for(File item : Objects.requireNonNull(inputFile.listFiles())) {
...
//		scene.getStylesheets().add(Objects.requireNonNull(MainRun.class.getResource("MainRun.css")).toString()); // For running in IDE
	...
	public void findDependencies() {
		...
		try{
			...
			Methods.logMessage(outputField, Arrays.toString(e.getStackTrace()));
		}
		Path dir = Paths.get(codeParent);
//		Methods.logMessage(outputField, codeLocation);  // Print location
			...
			for(String key : sampleDict.keySet()) {
				if(sampleDict.get(key).getFileType().equals("fastq")) {
					...
					if(currentSample.getFiles().size() == 2) {
						...
						fullProcessCall = new String[] {javaCall, "-ea", "-Xmx7g", "-cp", "./current", "jgi.BBDuk", "ref=" + ref, "k=" + klength,
								"in=" + currentSample.getFiles().getFirst(), "hdist=" + mismatches, "threads=" + threads, "interleaved=t",
